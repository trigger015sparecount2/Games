...
#define SMOKE_WIDTH (36.0f)  //煙の幅
#define SMOKE_HEIGHT (32.0f) //煙の高さ
...
void UpdateSmoke(void)
{
	...
	for (nCntSmoke = 0; nCntSmoke < MAX_SMOKE; nCntSmoke++)
	{
		...
		if (g_aSmoke[nCntSmoke].bUse == true)
		{
			...
			pVtx[0].pos = D3DXVECTOR3(g_aSmoke[nCntSmoke].pos.x - (SMOKE_WIDTH / 2.0f), g_aSmoke[nCntSmoke].pos.y - (SMOKE_HEIGHT / 2.0f), 0.0f);
			pVtx[1].pos = D3DXVECTOR3(g_aSmoke[nCntSmoke].pos.x + (SMOKE_WIDTH / 2.0f), g_aSmoke[nCntSmoke].pos.y - (SMOKE_HEIGHT / 2.0f), 0.0f);
			pVtx[2].pos = D3DXVECTOR3(g_aSmoke[nCntSmoke].pos.x - (SMOKE_WIDTH / 2.0f), g_aSmoke[nCntSmoke].pos.y + (SMOKE_HEIGHT / 2.0f), 0.0f);
			pVtx[3].pos = D3DXVECTOR3(g_aSmoke[nCntSmoke].pos.x + (SMOKE_WIDTH / 2.0f), g_aSmoke[nCntSmoke].pos.y + (SMOKE_HEIGHT / 2.0f), 0.0f);
		}
		pVtx += 4;
	}
	g_pVtxBuffSmoke->Unlock();
