...
void InitresultUi(void)
{
	...
	for (nCntnumber = 0; nCntnumber < 5; nCntnumber++)
	{
		...
		if (nCntnumber == 2 || TimeMin() == TIME_LIMIT_MIN)
		{
			pVtx[0].tex = D3DXVECTOR2(0.0f, 0.0f);
			pVtx[1].tex = D3DXVECTOR2(1.0f, 0.0f);
			pVtx[2].tex = D3DXVECTOR2(0.0f, 1.0f);
	...
	for (nCntrank = 0; nCntrank < 5; nCntrank++)
	{
		for (nCntnumber = 0; nCntnumber < 5; nCntnumber++)//残り時間
		{
			...
			if (nCntnumber == 2 || TRankData(nCntrank) == TIME_LIMIT_MIN * TIME_LIMIT_SEC)
			{
				pVtx[0].tex = D3DXVECTOR2(0.0f, 0.0f);
				pVtx[1].tex = D3DXVECTOR2(1.0f, 0.0f);
				pVtx[2].tex = D3DXVECTOR2(0.0f, 1.0f);
...
void UpdateresultUi(void)
{
	...
	if (g_nClearTime == 3)
	{
		for (nCntRank = 0; nCntRank < 5; nCntRank++)
		{
			if (TRankData(nCntRank) == TimeMin() * 60 + TimeSec() && TimeMin() != TIME_LIMIT_MIN && Demo() == false)
			{
				VERTEX_2D *pVtx;
				g_pVtxBuffresultUi->Lock(0, 0, (void **)&pVtx, 0);
...
void DrawresultUi(void)
{
	...
	for (nCntRank = 0; nCntRank < 5; nCntRank++)
	{
		for (nCntnumber = 0; nCntnumber < 5; nCntnumber++)
		{
			...
			if (nCntnumber != 2)
			{
				if (TRankData(nCntRank) == TIME_LIMIT_MIN * TIME_LIMIT_SEC)
				{
