...
void UpdateGamepad(void)
{
	...
	if (SUCCEEDED(g_pDevGamepad->GetDeviceState(sizeof(aPadState), &aPadState)))
	{
		...
		g_fCrossKey = g_aPadState.rgdwPOV[0];
...
float CrossKey(void)
{
	...
	if (g_aPadState.rgdwPOV[0] != 0xFFFFFFFF)
	{
		...
		if (g_fCrossKey != g_aPadState.rgdwPOV[0])
		{
...
{
	return (g_aPadState.rgbButtons[nButton] & 0x80) ? true : false;
}

//キーボードの入力情報を取得(押されたか離された)
bool GetGamePadTrigger(int nButton)
{
	return (g_aPadStateTrigger.rgbButtons[nButton] & 0x80) ? true : false;
...
{
	return (g_aPadStateRelease.rgbButtons[nButton] & 0x80) ? true : false;
}
...
{
	if ((g_aPadStateTrigger.rgbButtons[nButton] & 0x80) != 0 && (g_aPadStateRelease.rgbButtons[nButton] & 0x80) == 0)
	{
		return true;
	}
	return false;
