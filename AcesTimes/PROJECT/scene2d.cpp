...
HRESULT CScene2D::Init(D3DXVECTOR3 pos)
{
	...
	//頂点の座標
	pVtx[0].pos = D3DXVECTOR3(m_pos.x + sinf(m_fRot[0] * D3DX_PI) * m_Size.x, m_pos.y + cosf(m_fRot[0] * D3DX_PI) * m_Size.y, 0.0f);
	pVtx[1].pos = D3DXVECTOR3(m_pos.x + sinf(m_fRot[1] * D3DX_PI) * m_Size.x, m_pos.y + cosf(m_fRot[1] * D3DX_PI) * m_Size.y, 0.0f);
	pVtx[2].pos = D3DXVECTOR3(m_pos.x + sinf(m_fRot[2] * D3DX_PI) * m_Size.x, m_pos.y + cosf(m_fRot[2] * D3DX_PI) * m_Size.y, 0.0f);
	pVtx[3].pos = D3DXVECTOR3(m_pos.x + sinf(m_fRot[3] * D3DX_PI) * m_Size.x, m_pos.y + cosf(m_fRot[3] * D3DX_PI) * m_Size.y, 0.0f);
...
// ゲージ型のサイズ変更
//=============================================================================
void CScene2D::SetHGauge(D3DXVECTOR2 size)
{
	VERTEX_2D *pVtx;//頂点情報へのポインタ
...
//向きに足していく
void CScene2D::AddRot(float fRotY)
{
	int nCntRot;
...
//ポリゴンの大きさ変更
void CScene2D::SetSizeRot(float fRot)
{
	m_fRot[0] = -1.0f + fRot;
	m_fRot[1] = 1.0f - fRot;
	m_fRot[2] = -fRot;
...
//テクスチャ座標設定
void CScene2D::SetTex(D3DXVECTOR2 Scroll, D3DXVECTOR2 TipScroll)
{
	VERTEX_2D *pVtx;//頂点情報へのポインタ
	//頂点バッファをロックし、頂点データへのポインタを取得
	m_pVtxBuff->Lock(0, 0, (void**)&pVtx, 0);
	//テクスチャのUV座標
	pVtx[0].tex = D3DXVECTOR2(Scroll.x,		Scroll.y);
	pVtx[1].tex = D3DXVECTOR2(TipScroll.x,	Scroll.y);
	pVtx[2].tex = D3DXVECTOR2(Scroll.x,		TipScroll.y);
	pVtx[3].tex = D3DXVECTOR2(TipScroll.x,	TipScroll.y);
	//頂点バッファをアンロック
	m_pVtxBuff->Unlock();
}