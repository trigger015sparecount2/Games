...
void CCamera::AddPosPlus(D3DXVECTOR3 pos, D3DXVECTOR2 aCamera[2])
{
	...
	if (aCamera[1].x <= m_camera.posV.x + pos.x) //右に行き過ぎると限界で止まる
	{
		m_camera.posPlus.x = aCamera[1].x - ((m_camera.posV.x + pos.x) - m_camera.posPlus.x);
	}
	else if (m_camera.posV.x + pos.x <= aCamera[0].x) //左に行き過ぎると限界で止まる
	{
		m_camera.posPlus.x = aCamera[0].x - ((m_camera.posV.x + pos.x) - m_camera.posPlus.x);
	}
	if (UP_CAMERA <= m_camera.posPlus.y) //プレイヤーより上に行き過ぎると限界で止まる
	{
		m_camera.posPlus.y = UP_CAMERA;
	}
	else if (m_camera.posPlus.y <= UNDER_CAMERA) //プレイヤーより下に行き過ぎると限界で止まる
	{
		m_camera.posPlus.y = UNDER_CAMERA;
	}
	else if (aCamera[1].y <= m_camera.posV.y + pos.y) //上に行き過ぎると限界で止まる
	{
		m_camera.posPlus.y = aCamera[1].y - ((m_camera.posV.y + pos.y) - m_camera.posPlus.y);
	}
	else if (m_camera.posV.y + pos.y <= aCamera[0].y + m_pPlayer->GetHeight()) //下に行き過ぎると限界で止まる
	{
