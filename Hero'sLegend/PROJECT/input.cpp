...
bool GetAnyKey(void)
{
	...
	for (nCntKey = 0; nCntKey < NUM_KEY_MAX; nCntKey++)
	{
		if ((g_aKeyStateTrigger[nCntKey] & 0x80) != 0 && (g_aKeyStateRelease[nCntKey] & 0x80) == 0)
		{
			return true;
		}
	}
...
{
	if ((g_aKeyStateTrigger[nKey] & 0x80) != 0 && (g_aKeyStateRelease[nKey] & 0x80) == 0)
	{
		return true;
	}
	return false;
