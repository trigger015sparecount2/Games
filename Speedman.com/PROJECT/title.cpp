...
void CTitle::Update()
{
	...
	if (Fade == CFade::FADE_NONE)
	{
		...
		if (m_pGamePad != NULL)
		{
			...
			if (m_bFade == false)
			{
				...
				if (CrossKey == 0.0f)
				{
					...
					if (m_nSelectNum <= 0)
					{
						m_nSelectNum = MAX_TITLESERECT - 1;
					}
				...
				{
					m_nSelectNum += 1;
					if (m_nSelectNum >= MAX_TITLESERECT)
					{
						m_nSelectNum = 1;
					}
					CSound::Play(CSound::SOUND_LABEL_SELECT);
		...
		else if (m_pKeyboard != NULL)
		{
			...
			//{
			//シーン遷移
			if ((m_pKeyboard->GetKey(DIK_RETURN) == true || m_pKeyboard->GetKey(DIK_SPACE) == true) && m_nSelectNum == 1)
			{
				CFade::SetFade(CManager::MODE_GAME);
				CScore::SetTime(TIME_LIMIT);
				CManager::SetGameEnd(false);
			...
			else if ((m_pKeyboard->GetKey(DIK_RETURN) == true || m_pKeyboard->GetKey(DIK_SPACE) == true) && m_nSelectNum == 2)
			{
				CFade::SetFade(CManager::MODE_TUTORIAL);
				CManager::SetGameEnd(false);
				CManager::SetSaveRankOne(false);
			...
			else if ((m_pKeyboard->GetKey(DIK_RETURN) == true || m_pKeyboard->GetKey(DIK_SPACE) == true) && m_nSelectNum == 3)
			{
				m_bFade = true;
				CFade::SetFade(CManager::MODE_RESULT);
				CManager::SetGameEnd(false);
