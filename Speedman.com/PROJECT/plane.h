...
public:
	...
	PDIRECT3DVERTEXBUFFER9 GetVtx() { return m_pVtxBuff; }	//頂点バッファ取得
	void SetTexture(int nTex);			//指定された番号のテクスチャを貼れるように
	void ChangeColor(D3DXCOLOR col);	//色替え

	//エフェクト用関数
	void ColorChange(D3DCOLORVALUE color);		//色変更
	void ChangeSize(D3DXVECTOR3 size);			//サイズ変更
	static CPlane *Create(D3DXVECTOR3 size, D3DXVECTOR3 pos, D3DXVECTOR2 Tex);	//作成
